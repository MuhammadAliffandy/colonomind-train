...
def _hm(p, r): return round(2*p*r/(p+r), 2)

# Data order: [Accuracy%, Precision%, Recall%, F1=HM(P,R), QWK]
# F1 recomputed as harmonic mean of Precision & Recall for full mathematical consistency.
# Accuracy, Precision, Recall, and QWK are the original reported values (unchanged).
BASE_DATA = {
    'NTUH': {
        'ResNet-50':       [70.35, 66.99, 67.17, _hm(66.99,67.17), 0.7957],
        'DenseNet-121':    [67.34, 64.41, 64.15, _hm(64.41,64.15), 0.7906],
        'EfficientNet-B4': [77.89, 75.65, 76.03, _hm(75.65,76.03), 0.8797],
        'ConvNeXt-Tiny':   [71.36, 67.85, 68.04, _hm(67.85,68.04), 0.8350],
        'ViT-B/16':        [48.74, 47.14, 46.85, _hm(47.14,46.85), 0.4330],
    },
    'TMC-UCM': {
        'ResNet-50':       [79.94, 78.79, 78.34, _hm(78.79,78.34), 0.9201],
        'DenseNet-121':    [78.56, 77.70, 77.27, _hm(77.70,77.27), 0.9134],
        'EfficientNet-B4': [83.74, 83.05, 82.83, _hm(83.05,82.83), 0.9354],
        'ConvNeXt-Tiny':   [80.85, 79.69, 79.31, _hm(79.69,79.31), 0.9252],
        'ViT-B/16':        [48.10, 45.51, 44.65, _hm(45.51,44.65), 0.4618],
    },
    'LIMUC': {
        'ResNet-50':       [76.57, 68.40, 69.07, _hm(68.40,69.07), 0.8415],
        'DenseNet-121':    [75.50, 66.34, 67.70, _hm(66.34,67.70), 0.8403],
        'EfficientNet-B4': [78.11, 71.46, 73.48, _hm(71.46,73.48), 0.8553],
        'ConvNeXt-Tiny':   [76.87, 61.16, 61.28, _hm(61.16,61.28), 0.7353],
        'ViT-B/16':        [68.62, 58.91, 58.72, _hm(58.91,58.72), 0.7058],
    },
    'Unified': {
        'ResNet-50':       [76.91, 74.54, 72.85, _hm(74.54,72.85), 0.8609],
        'DenseNet-121':    [73.87, 71.22, 68.95, _hm(71.22,68.95), 0.8220],
        'EfficientNet-B4': [74.59, 71.01, 70.85, _hm(71.01,70.85), 0.8485],
        'ConvNeXt-Tiny':   [79.13, 77.54, 75.16, _hm(77.54,75.16), 0.8911],
        'ViT-B/16':        [74.41, 71.45, 70.70, _hm(71.45,70.70), 0.8319],
    }
}
...
def main():
    ...
    for d_idx, ds in enumerate(['NTUH','TMC-UCM','LIMUC','Unified'],1):
        ...
        for model, v in BASE_DATA[ds].items():
            acc,prec,rec,f1,qwk = v  # Stored order: [Acc, Prec, Rec, F1, QWK]
            rows.append({
                'Model': model,
