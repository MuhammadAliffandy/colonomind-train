...
def main():
    ...
    print("Loading Teacher Probabilities for Knowledge Distillation...")
    teacher_dir = os.path.join(BASE_SAVE_DIR.replace(f"/{args.model}_Experiment", "/Teacher_Probs"))
    teacher_train_probs = np.load(os.path.join(teacher_dir, "teacher_train_probs.npy"))
    teacher_val_probs = np.load(os.path.join(teacher_dir, "teacher_val_probs.npy"))
    
    # Label Blending
    alpha = 0.3 # 30% Hard label, 70% Soft Teacher Probabilities
    print(f"Blending targets: {alpha*100}% Hard Labels + {(1-alpha)*100}% Teacher Soft Labels")
    y_train_kd = alpha * y_train_cat + (1 - alpha) * teacher_train_probs
    y_val_kd = alpha * y_val_cat + (1 - alpha) * teacher_val_probs
    ...
    history = model.fit(
        [X_img_train, X_feat_train_scaled, X_train_umap], y_train_kd,
        validation_data=([X_img_val, X_feat_val_scaled, X_val_umap], y_val_kd),
        batch_size=BATCH_SIZE, epochs=EPOCHS, class_weight=class_weight_dict, callbacks=callbacks, verbose=1
    )

    model_path = os.path.join(BASE_SAVE_DIR, f"{args.model}_hybrid_kd.keras")
