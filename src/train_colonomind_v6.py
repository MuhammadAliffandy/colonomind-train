"""
ColonoMind v6 — Mod-SE CNN + Deep Ensembling + Super Agent
============================================================
Base: Mod-SE CNN (Strictly no pre-trained weights)
Optimisations for 90% F1/Precision/Recall:
  1. Deep Ensembling: Train 3 separate Mod-SE CNNs with different random seeds.
     Average their deep features to bypass single-model variance limitations.
  2. Loss Function: Reverted to Categorical Cross-Entropy with Label Smoothing (0.1)
     to maximise exact match accuracy (F1-score) instead of ordinality.
  3. Feature-Space SMOTE: Applied SMOTE to the 643-dim Super Agent features
     before training LightGBM to fully balance MES1 and MES3.
  4. CutMix augmentation (20%).
  5. 3-Phase training: Warmup(30) → Mid(60) → Full(120).
  6. Per-class confidence threshold routing.
  7. Optuna 30 trials × 5-Fold CV on Macro F1.
  8. F1-Maximizer (differential evolution post-processing).
"""
...
def main():
    ...
    ap.add_argument("--save_dir", default="../Result/ColonoMind_v6")
    ...
    print("="*70); print("🚀 COLONOMIND v6 — Mod-SE CNN + Deep Ensembling + Super Agent"); print("="*70)
    ...
    # ── 3. UMAP & Scaling ──────────────────────────────────────────────────────
    sp, up = os.path.join(args.save_dir,"scaler_v6.pkl"), os.path.join(args.save_dir,"umap_v6.pkl")
    ...
    # ── 4. Deep Ensembling Training (3 Models) ─────────────────────────────────
    print(f"\n🧬 Deep Ensembling: Training {len(ENSEMBLE_SEEDS)} models with seeds {ENSEMBLE_SEEDS}")
    loss_fn = CategoricalCrossentropy(label_smoothing=0.1)  # Reverted to CCE for F1 optimization
    ...
    for m_idx, seed in enumerate(ENSEMBLE_SEEDS):
        mp = os.path.join(args.save_dir, f"secnn_v6_seed{seed}.h5")
        if os.path.exists(mp):
            print(f"\n⚡ Found checkpoint for Seed {seed} at {mp}. Skipping training.")
            model = load_model(mp, compile=False)
        else:
            print(f"\n🔥 Training CNN Model {m_idx+1}/{len(ENSEMBLE_SEEDS)} (Seed: {seed})")
            model = build_se_cnn_hybrid(seed)
            # Phase 1
            model.compile(optimizer=Adam(5e-4), loss=loss_fn, metrics=['accuracy'])
            model.fit(tr_gen, validation_data=va_gen, epochs=args.epochs_warmup, class_weight=cw_dict,
                      callbacks=[ModelCheckpoint(mp, save_best_only=True, monitor='val_accuracy', mode='max'), CosineAnneal(5e-4, 1e-5, args.epochs_warmup)], verbose=1)
            # Phase 2
            model.compile(optimizer=Adam(2e-4), loss=loss_fn, metrics=['accuracy'])
            model.fit(tr_gen, validation_data=va_gen, epochs=args.epochs_mid, class_weight=cw_dict,
                      callbacks=[ModelCheckpoint(mp, save_best_only=True, monitor='val_accuracy', mode='max'), EarlyStopping(monitor='val_accuracy', patience=15, restore_best_weights=True, mode='max'), CosineAnneal(2e-4, 1e-6, args.epochs_mid)], verbose=1)
            # Phase 3
            model.compile(optimizer=Adam(1e-4), loss=loss_fn, metrics=['accuracy'])
            model.fit(tr_gen, validation_data=va_gen, epochs=args.epochs_full, class_weight=cw_dict,
                      callbacks=[ModelCheckpoint(mp, save_best_only=True, monitor='val_accuracy', mode='max'), EarlyStopping(monitor='val_accuracy', patience=20, restore_best_weights=True, mode='max'), ReduceLROnPlateau(monitor='val_loss', factor=0.5, patience=5)], verbose=1)
            model = load_model(mp, compile=False)
    ...
    joblib.dump(ag_sc, os.path.join(args.save_dir, "agent_scaler_v6.pkl"))
    ...
    best_p.update(dict(objective='multiclass', num_class=NUM_CLASSES, metric='multi_logloss', verbosity=-1, class_weight='balanced', n_jobs=8))
    ...
    final_agent.booster_.save_model(os.path.join(args.save_dir, "super_agent_v6.txt"))
